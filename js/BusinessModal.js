...
export default class BusinessModal extends Component {
    ...
    if (Object.keys(this.state.business).length !== 0) {
      return (
        <View style={localStyles.inner}>
          ...
          >
            ...
            >
              ...
              {this.state.business.address ? (
                <Text
                  style={localStyles.businessInfo}
                >
                  {this.state.business.address}
                </Text>
              ) : null}
          ...
          <TouchableHighlight
            style={localStyles.yelpButton}
            underlayColor={'#68a0ff'}
            onPress={this._yelpLink}
          >
            <Image
              source={require('./res/Yelp_burst_positive_RGB_2.png')}
              resizeMode="stretch"
              style={{ flex: 1, height: 30, width: 100, margin: 'auto' }}
            />
          </TouchableHighlight>
          {this.state.business.isOpen === null ? null : this.state.business
            .isOpen ? (
            <Text style={localStyles.open}>Open now</Text>
          ) : (
            <Text style={localStyles.closed}>Closed now</Text>
          )}
          <View style={localStyles.openingTimes}>
            {this.state.business.hours.map((hours, i) => {
              return (
                <View style={localStyles.day}>
                  <Text style={localStyles.text}>{lookupDays[i]} </Text>
                  <Text style={localStyles.hours}>{hours}</Text>
                </View>
              );
            })}
          </View>
          {this.state.business.foodRating ? (
            <Image
              style={{ width: 115, height: 58, margin: 5 }}
              source={hygiene_rating[this.state.business.foodRating]}
            />
          ) : null}
          <TouchableHighlight
  ...
  _fetchData = () => {
    let businessName = this.props.business.name
      ...
      .replace(/&/g, 'and');
    fetch(
      `https://0p83k3udwg.execute-api.us-east-1.amazonaws.com/dev/api/device/business/${
  ...
  _yelpLink = () => {
    Linking.openURL(this.state.business.url).catch(() =>
      this.props.displayError()
    );
  };
}
